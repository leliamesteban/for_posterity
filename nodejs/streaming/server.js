...
fs.readFile(path.resolve(__dirname,"album.webm"), function (err, data) {
    if (err) {
        throw err;
    }
    movie_webm = data;
});
fs.readFile(path.resolve(__dirname,"KMD.mp4"), function (err, data) {
    if (err) {
        throw err;
    }
    movie_mp4 = data;
...
fs.readFile(path.resolve(__dirname,"home.html"), function (err, data) {
    if (err) {
        throw err;
    }
    indexPage = data;    
...
http.createServer(function (req, res) {
    ...
    if(reqResource == "/"){
        ...
        res.end();
            ...
            var total;
            if(reqResource == "/KMD.mp4"){
                total = movie_mp4.length;
            } else if(reqResource == "/album.webm"){
